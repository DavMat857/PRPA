...
Esta formaría parte de la opción segura, debido a que :

Los coches del norte pueden pasar siempre que no haya coches del sur ni peatones

Los coches del sur pueden pasar siempre que no haya coches del norte ni peatones

Los peatones pueden pasar siempre que no hayan coches
...
#############COCHES
    # Método para solicitar entrada al puente para un coche
    def wants_enter_car(self, direction: int) -> None:
        self.mutex.acquire() # Adquiere el candado para proteger el acceso a las variables compartidas

        # Incrementa el valor de la variable patata
        self.patata.value += 1

        # Si el coche va en dirección Norte
        if direction == NORTH:
            # Espera hasta que pueda pasar un coche en dirección Norte
            self.can_cars_N.wait_for(self.can_pass_cars_N)
            self.ncars_N.value +=1 # Incrementa el número de coches en dirección Norte

        # Si el coche va en dirección Sur
        else:
            # Espera hasta que pueda pasar un coche en dirección Sur
            self.can_cars_S.wait_for(self.can_pass_cars_S)
            self.ncars_S.value+=1 # Incrementa el número de coches en dirección Sur

        self.mutex.release() # Libera el candado

    # Método para indicar que un coche sale del puente
    def leaves_car(self, direction: int) -> None:
        self.mutex.acquire() # Adquiere el candado para proteger el acceso a las variables compartidas
        self.patata.value += 1 # Incrementa el valor de la variable patata

        # Si el coche va en dirección Norte
        if direction==NORTH:
            self.ncars_N.value-=1 # Decrementa el número de coches en dirección Norte
            # Si ya no hay coches en dirección Norte
            if self.ncars_N.value==0:
                # Notifica a coches en dirección Sur y peatones que pueden pasar
                self.can_cars_S.notify_all()
                self.can_ped.notify_all()

        # Si el coche va en dirección Sur
        else:
            self.ncars_S.value-=1 # Decrementa el número de coches en dirección Sur
            # Si ya no hay coches en dirección Sur
            if self.ncars_S.value==0:
                # Notifica a coches en dirección Norte y peatones que pueden pasar
...
#############PEATONES
    # Método para solicitar entrada al puente para un peatón
    def wants_enter_pedestrian(self) -> None:
        self.mutex.acquire() # Adquiere el candado para proteger el acceso a las variables compartidas
        self.patata.value += 1 # Incrementa el valor de la variable patata

        # Espera hasta que pueda pasar un peatón
        self.can_ped.wait_for(self.can_pass_ped)
        self.nped.value+=1 # Incrementa el número de peatones
        self.mutex.release() # Libera el candado

    # Método para indicar que un peatón sale del puente
    def leaves_pedestrian(self) -> None:
        self.mutex.acquire() # Adquiere el candado para proteger el acceso a las variables compartidas
        self.patata.value += 1 # Incrementa el valor de la variable patata
        self.nped.value-=1 # Decrementa el número de peatones

        # Si ya no hay peatones
        if self.nped.value==0:
            # Notifica a coches en dirección Norte y Sur que pueden pasar
            self.can_cars_N.notify_all()
            self.can_cars_S.notify_all()

        self.mutex.release() # Libera el candado

    # Verifica si pueden pasar coches en dirección Sur
    def can_pass_cars_S(self) -> bool:
        # Retorna verdadero si no hay coches en dirección Norte ni peatones
        return self.ncars_N.value == 0 and self.nped.value == 0

    # Verifica si pueden pasar coches en dirección Norte
    def can_pass_cars_N(self) -> bool:
        # Retorna verdadero si no hay coches en dirección Sur ni peatones
        return self.ncars_S.value == 0 and self.nped.value == 0

    # Verifica si pueden pasar peatones
    def can_pass_ped(self) -> bool:
        # Retorna verdadero si no hay coches en dirección Norte ni en dirección Sur
